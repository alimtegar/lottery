...
const App = () => {
	...
	const [items, setItems] = useState<Item[]>([{ id: Math.random(), title: '', frame: '', n: null }]);
	...
	const addItem = () => setItems([
		...items,
		{ id: Math.random(), title: '', frame: '', n: null },
	]);
	...
	[
		...
		numberedItem = [
			{ title: 'Packet A', frame: '1', n: 5, lotteryNumbers: [11,12,13,14,15] },
			{ title: 'Packet B', frame: '2', n: 4, lotteryNumbers: [21,22,23,24] },
			{ title: 'Ricecooker', frame: '3', n: 1, lotteryNumbers: [31] },
			{ title: 'Fridge', frame: '3', n: 1, lotteryNumbers: [32] },
		]

		results = [
			{ frame: '1', numberedItems: [ { title: 'Packet A', frame: '1', n: 5, lotteryNumbers: [11,12,13,14,15] } ] }
			{ frame: '2', numberedItems: [ { title: 'Packet B', frame: '2', n: 5, lotteryNumbers: [11,12,13,14,15] } ] }
			{ frame: '3', numberedItems: [ { title: 'Ricecooker', frame: '3', n: 1, lotteryNumbers: [31] }, { title: 'Fridge', frame: '3', n: 1, lotteryNumbers: [32] } ] }
		]
	]
	*/
			...
			<form className="bg-white w-full lg:w-5/12 p-4 shadow">
				<div className="flex flex-col min-h-full">
							...
							{items.map((item, i) => (
								<React.Fragment key={item.id}>
									<input
										...
										className="input input-bordered w-full text-xs col-span-8 sm:col-span-3"
										value={item.title}
									...
									<input
										...
										placeholder="Jumlah"
										min="1"
										className="input input-bordered w-full text-xs col-span-3 sm:col-span-2"
										value={item.n ? +item.n : ''}
										onChange={(e) => handleItemChange(item.id, e)}
									/>
									<input
									...
									{items.length > 1 ? (
										<button type="button" onClick={() => removeItem(item.id)} className="btn btn-outline btn-error col-span-2 sm:col-span-1">
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
												<path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
											</svg>
										</button>
									) : (
										<button type="button" className="btn btn-outline btn-error col-span-2 sm:col-span-1" disabled>
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
												<path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
											</svg>
										</button>
					...
					<div className="form-control w-full mt-auto">
						{(
							range &&
							items.every((item) => item.title && item.frame && item.n && item.n > 0)
						) ? (
